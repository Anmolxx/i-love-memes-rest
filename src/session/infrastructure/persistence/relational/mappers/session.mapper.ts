...
export class SessionMapper {
  ...
  static toPersistence(domainEntity: Session): SessionEntity {
    ...
    // user.id may be a UUID string; preserve it instead of coercing to Number
    user.id = domainEntity.user.id as any;

    const persistenceEntity = new SessionEntity();
    if (
      domainEntity.id &&
      (typeof domainEntity.id === 'number' ||
        typeof domainEntity.id === 'string')
    ) {
      persistenceEntity.id = domainEntity.id as any;
    }
